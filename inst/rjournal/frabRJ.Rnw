...
\cite{hankin2022_disordR}


\section{Prologue: \code{table()}}
...
language [@rcore2022].  However, consider the following two named
vectors:
...
pairwise, and the \code{names} attribute is taken from the {\em first}
...
\section{The \CRANpkg{frab} package}

The \CRANpkg{frab} package associates a numerical value with each of a
set of arbitrary (character string) symbols.  This is accomplished
using the \code{STL} map class, a container that stores key-value
...
order, as per \CRANpkg{disordR} discipline (discussed below).
Further, note that \code{a+b} has length 4, while \code{a} and
\code{b} have length 3.

\subsection{Extensions of \code{frab} objects to floating-point values}

The \code{frab} class is sufficiently flexible to incorporate
floating-point values, although one has to be a little careful with
numerical round-off errors:

<<showfrabroundofferrors>>=
...
Above, the sum \code{x+y+z} should have zero entries for \code{u} and
\code{p}; we see the entry for \code{u} vanishes, but the entry for
\code{p} is nonzero, being subject to (small) numerical roundoff
error.

\section{The \CRANpkg{frab} package and \CRANpkg{disordR} discipline}

The \CRANpkg{frab} package conforms to disord discipline
\citep{hankin2022_disordR}.  Here I present some discussion of the
motivation for this design decision in the context of the
\CRANpkg{frab} package; a more general discussion is given in
...
Above we see a \CRANpkg{disordR} violation: there is no such thing as
"\code{a[1]}", for the elements of \code{a} are stored in an
implementation-specific order.  The \code{STL} map class stores value-key
pairs in an undefined order; thus, extracting the value for \code{x} is
