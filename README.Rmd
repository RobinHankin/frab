---
title: "The frab package: how to add two tables"
output:
  github_document:
    pandoc_args: --webtex
---
...
primary key.  Support for multi-dimensional tables is included.
...
Class `frab` is for one-dimensional tables and is an alternative
implementation of named vectors; class `sparsetable` handles
multi-way tables in a natural way.
...
Above we see function `rfrab()` used to generate a random `frab` object, corresponding to a table.  It is _possible_ to add `x` and `y` directly:
...
possible entries, only 11 are non-zero.  We may coerce to a regular table:
...
The random `sparsetable` object shown above would require $10^{12}$ floating point numbers in full array form,
of which only 4 are nonzero.  Multi-way tables may be added in the same way as `frab` objects:
...
Two-way tables are something of a special case, having their own print method.
